...
def train_model(params, best_val_fscore):
    ...
    )  # BertTokenizer.from_pretrained(params['path_files'], do_lower_case=False)
    df_train = pd.read_csv(
        params["train_data"]
    )  # data_collector(train_files,params,True)
    df_val = pd.read_csv(
        params["validate_data"]
    )  # data_collector(val_files,params,False)
    ...
    labels_train = df_train["hate_speech"].values
    labels_val = df_val["hate_speech"].values
    label_counts = df_train["hate_speech"].value_counts()
    print(label_counts)
    ...
    model = BertForSequenceClassification.from_pretrained(
        "bert-base-multilingual-uncased"
    )  # select_model(params['what_bert'],params['path_files'],params['weights'])
    ...
    input_val_ids, att_masks_val = combine_features(
        sentences_val, tokenizer, params["max_length"]
    )
    ...
    validation_dataloader = return_dataloader(
        input_val_ids,
        labels_val,
        att_masks_val,
        batch_size=params["batch_size"],
